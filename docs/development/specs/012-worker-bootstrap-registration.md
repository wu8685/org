...
> Terminology: this specification follows the canonical [org glossary](../../user/architecture/glossary.md). Product identity is Tenant → Worker → WorkerVersion. Infrastructure continues to use one shared platform Temporal Namespace and one shared platform Kubernetes Namespace; a product Tenant is never mapped to either underlying platform resource boundary.
