...
> Terminology follows the canonical [org glossary](../../user/architecture/glossary.md). Product identity remains Tenant → Worker → WorkerVersion → Workflow Run. Infrastructure continues to use one shared platform Temporal Namespace and one shared platform Kubernetes Namespace.
