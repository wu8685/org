...
> Terminology: this status follows the canonical [org glossary](../user/architecture/glossary.md). Product isolation is a Tenant; infrastructure uses one shared platform Temporal Namespace and one shared platform Kubernetes Namespace.
...
- all three Samples are self-contained Worker repositories with their own versioned Go module, Makefile, Docker context, build/push/kind scripts and value-first README; the publish request example is centralized under `docs/user/api`, and root targets only delegate;
...
- Console Tenant selector exposes only the authenticated principal's authorized memberships, uses durable session selection across refresh/restart, and reroutes all Worker/Version/Workflow/Run reads and mutations through the newly authenticated Tenant context without client-provided Tenant locators;
- Console Tenant management provides authorized-only listing, immutable-slug creation, revision-safe metadata/quota updates, server-derived role membership management, last-owner protection, dynamic selector refresh/fallback and durable Tenant-scoped Audit; the loopback executable accepts only a server-configured principal catalog and does not impersonate an external identity system;
...
- Console Tenant selection acceptance passed authorized-membership filtering, CSRF and forged-locator rejection, durable restart/fallback, same-name cross-Tenant routing, single/multiple membership UI contracts, hierarchical breadcrumbs and a real two-Tenant process-level HTTP smoke; root/Sample race/vet/docs and all three real kind + Temporal paths remained green.
...
- Tenant management acceptance passed atomic FileStore restart/failure injection, authorized-only service/API reads, role/last-owner/quota/ETag/CSRF checks, dynamic session fallback, and a real local browser flow covering creation, automatic selection, metadata update, existing-principal membership, unknown-principal rejection and 390px mobile layout; root/Sample race/vet/docs and all three real kind + Temporal acceptance paths remained green.
