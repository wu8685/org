...
func (r *acceptanceRun) buildAndLoadSample(suffix string) sampleImage {
	...
	cmd := exec.CommandContext(r.ctx, "make", "-C", r.root, "sample-kind-load", "SAMPLE_VERSION="+version, "SAMPLE_COMMIT="+commit)
	cmd.Env = os.Environ()
	...
	if digest == "" || tag == "" {
		r.t.Fatalf("sample-kind-load did not print tag and digest reference:\n%s", output)
	}
